...
void Ki3Tiler::registerShortcuts()
{
    ...
    add(QStringLiteral("ki3_resize_shrink_h"), i18n("ki3: Shrink Width"),
        ...
        [this]() { resizeActive(Qt::Horizontal, -50); });
    ...
    add(QStringLiteral("ki3_resize_grow_v"), i18n("ki3: Grow Height"),
        ...
        [this]() { resizeActive(Qt::Vertical, 50); });
    ...
    add(QStringLiteral("ki3_spawn_terminal"), i18n("ki3: Launch Terminal"),
        {QKeySequence(Qt::META | Qt::Key_Return)}, [this]() { spawnTerminal(); });
    ...
    add(QStringLiteral("ki3_split_horizontal"), i18n("ki3: Split Horizontal"),
        ...
        [this]() { setSplitDirection(Tile::LayoutDirection::Horizontal); });
    add(QStringLiteral("ki3_split_vertical"), i18n("ki3: Split Vertical"),
        {QKeySequence(Qt::META | Qt::Key_V)},
        [this]() { setSplitDirection(Tile::LayoutDirection::Vertical); });
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        add(QStringLiteral("ki3_workspace_%1").arg(n), i18n("ki3: Switch to Workspace %1", n),
            {QKeySequence(Qt::META | digits[i])}, [this, n]() { switchToWorkspace(n); });
        add(QStringLiteral("ki3_move_workspace_%1").arg(n), i18n("ki3: Move to Workspace %1", n),
            {QKeySequence(Qt::META | Qt::SHIFT | digits[i])}, [this, n]() { moveActiveToWorkspace(n); });
    }
}
...
void Ki3Tiler::redistributeEvenly(CustomTile *parent)
{
    ...
    qCDebug(KWIN_KI3) << "redistribute" << n << (horizontal ? "H" : "V")
                      << "->" << [&] { QList<RectF> r; for (Tile *t : children) r << t->relativeGeometry(); return r; }();
}
...
void Ki3Tiler::moveFocusAcrossOutput(CustomTile *leaf, Qt::Edge edge)
{
    ...
    switch (edge) {
    case Qt::LeftEdge:   probe = {geom.left() - 1.0, leafGeom.center().y()}; break;
    case Qt::RightEdge:  probe = {geom.right() + 1.0, leafGeom.center().y()}; break;
    case Qt::TopEdge:    probe = {leafGeom.center().x(), geom.top() - 1.0}; break;
    case Qt::BottomEdge: probe = {leafGeom.center().x(), geom.bottom() + 1.0}; break;
    }
    ...
    switch (edge) {
    case Qt::LeftEdge:   entry = {ngeom.right() - 2.0, std::clamp(leafGeom.center().y(), ngeom.top(), ngeom.bottom() - 1.0)}; break;
    case Qt::RightEdge:  entry = {ngeom.left() + 2.0, std::clamp(leafGeom.center().y(), ngeom.top(), ngeom.bottom() - 1.0)}; break;
    case Qt::TopEdge:    entry = {std::clamp(leafGeom.center().x(), ngeom.left(), ngeom.right() - 1.0), ngeom.bottom() - 2.0}; break;
    case Qt::BottomEdge: entry = {std::clamp(leafGeom.center().x(), ngeom.left(), ngeom.right() - 1.0), ngeom.top() + 2.0}; break;
    }
