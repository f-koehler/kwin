...
Ki3Tiler::~Ki3Tiler()
{
    // A clean teardown so the plugin can be unloaded -- and, in particular,
    // hot-reloaded (see hot-reload-ki3.sh) -- without leaving debris in KWin's
    // *shared* state that outlives this object: windows stranded borderless and
    // still tile-controlled, or orphaned CustomTiles piling up in the per-output
    // TileManagers across reloads. Everything ki3 *owns* -- the overlay windows
    // (split indicator, resize border, tile borders), the /Ki3 D-Bus object, the
    // KGlobalAccel QActions, and every signal connection whose receiver is this
    // object -- is a member or child QObject and is released automatically as this
    // object is destroyed, so it is deliberately not repeated here.
    qCInfo(KWIN_KI3) << "ki3 tiling plugin unloading";

    // Floating windows: drop ki3's chrome and restore their native decoration and
    // normal stacking (createFloatChrome() had set noBorder + keepAbove).
    const QList<Window *> floating(m_floatingWindows.cbegin(), m_floatingWindows.cend());
    ...
    // Tab/stack group headers before the tiles they sit on.
    const QList<CustomTile *> groups = m_tabbed.keys();
    for (CustomTile *tile : groups) {
        destroyGroupHeader(tile);
    }
    m_tabbed.clear();

    // Dismantle every tile tree ki3 took over. purgeStaleRoots() first drops roots
    // whose output was unplugged (their tiles are already gone with the
    // TileManager); the RootTile itself is KWin's, so we keep it and only undo what
    // we did to it. For each surviving root we detach every window from its tile
    // -- forget() drops it from the tile's window list *and* clears its requested
    // tile, so removing the now-empty tiles below can't re-home it into a sibling
    // (CustomTile::remove() migrates any windows it still holds) -- restore its own
    // title bar, then remove all the child tiles we created so none linger.
    purgeStaleRoots();
    for (RootTile *root : std::as_const(m_managedRoots)) {
        root->visitDescendants([](Tile *t) {
            auto *tile = static_cast<CustomTile *>(t);
            const QList<Window *> windows = tile->windows();
    ...
    // Restore the title bar of any managed window whose root was already gone (its
    // tiles vanished with an unplugged output, so the loop above never saw it).
    for (auto it = m_leafForWindow.cbegin(); it != m_leafForWindow.cend(); ++it) {
    ...
    // The global "mode" state ki3 asserts -- per-output virtual desktops
    // (setPerOutputVirtualDesktops) and the disabled Overview hot corner
    // (disableOverviewHotCorner) -- is deliberately left in place: a hot-reload's
    // next load re-asserts both immediately, and tearing them down here would churn
    // the user's desktops (and the windows on them) for no benefit during the
    // reload gap. A permanent unload leaves them as ki3 last set them.
}
